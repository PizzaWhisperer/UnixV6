...
//correcteur : Très bon job d'avoir réussi à corriger pas mal de vos erreurs du rendu 1 dans le rendu 2 sans avoir eu formellement de corrigés ou de feedback !
...
static int arg_parse(void* data, const char* filename, int key, struct fuse_args* outargs)
{
    ...
    if (key == FUSE_OPT_KEY_NONOPT && fs.f == NULL && filename != NULL) {
        ...
        if (feedback) {
            //correcteur : très bien d'utiliser stderr pour ce genre de choses (0.5 points de bonus)
...
static int fs_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
    ...
    //correcteur : vous ne testez pas si l'inode que vous avez lue n'est pas un file (-1)
