...
int direntv6_create(struct unix_filesystem *u, const char *entry, uint16_t mode)
{
    ...
    char entry_cpy[MAXPATHLEN_UV6 + 1];
    memset(entry_cpy, 0, MAXPATHLEN_UV6 + 1);
    strncpy(entry_cpy, entry, strlen(entry));
    entry_cpy[MAXPATHLEN_UV6] = '\0';

    char* parent = trim_slash(entry_cpy);
    ...
    *child = '\0';
    ...
    int feedback = direntv6_dirlookup(u, ROOT_INUMBER, parent);
    if (feedback < 0) {
        return ERR_BAD_PARAMETER;
    }

    feedback = direntv6_dirlookup(u, (uint16_t) feedback, child);
    if (feedback >= 0) {
        return ERR_FILENAME_ALREADY_EXISTS;
    }

    /// Get the next free inode and write there.
    int inodeNumber = inode_alloc(u);
    if (inodeNumber < 0) {
        return inodeNumber;
    }
