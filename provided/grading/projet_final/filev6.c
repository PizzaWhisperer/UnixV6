 ...
 * @return 0 on success; <0 on error
 */
//correcteur : c'est une inteprétation valide de la consigne qui n'avait pas été envisagée. En "théorie" on s'attendait à ce que les élèves s'occupent dans cette fonction de traiter tout les cas d'écrire les secteurs incomplets, passer d'un petit fichier au grand, etc... Et ainsi avoir writebytes qui appelle juste en boucle writesector jusqu'à ce que tout le contenu soit écrit. Mais vous faites tout ces cas non triviaux dans writebytes, donc c'est totalement valide comme implémentation.
...
{
    //correcteur : votre fonction était très bien indentée et très bien commentée, (et bien évidemment bien codée également) c'était très facile à vous lire et à vous comprendre dans votre code (malgré le fait qu'il fasse 150+ lignes) (1 point bonus)
    M_REQUIRE_NON_NULL(u);
    M_REQUIRE_NON_NULL(u->f);
    M_REQUIRE_NON_NULL(fv6);
    M_REQUIRE_NON_NULL(buf);
