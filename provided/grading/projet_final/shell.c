...
//correcteur : vous avez fait globalement un très bon boulot pour corriger toutes les erreurs que j'avais relevé dans ma correction du projet01 (Bravo !). Si jamais je n'ai pas remis les commentaires de corrections pour les points qui n'avaient pas été corrigés, donc si vous voyez qu'il y a une partie du projet01/2 dans votre fichier de points qui manque des points et que vous ne voyez pas de commentaire de ma part, ça signifie que vous n'avez pas corrigé cette erreur et mon commentaire correspondant se trouve dans la correction du projet01/2.
...
int do_add(const char** array)
{
    ...
    //correcteur : vous oubliez de close le file ici. (-1)
