...
#define CLEAR (0)
//correcteur : marrant comme nom de macro. Ya une histoire la derrière ?
#define GET_SHADOW (1)
#define BYTE_SIZE (8)
...
void bm_print(struct bmblock_array *bmblock_array)
{
    ...
    if (bmblock_array != NULL) {
        ...
        //correcteur : ces valeurs ci dessous sont de type uint64_t, il aurait fallut utiliser le format correct pour les imprimer (PRIu64) (-0.5)
        printf("min: %zu\n", bmblock_array->min);
        printf("max: %zu\n", bmblock_array->max);
